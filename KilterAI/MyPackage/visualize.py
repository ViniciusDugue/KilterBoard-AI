...
def draw_colored_circles_on_image(image_path, circles, save_as='KilterBoardSetup1.png'):
    ...
    for x, y, z in circles:
        if z not in hold_colors:
            print(f"Warning: z value {z} is not in hold_colors dictionary")
...
def frame_to_image(frame):
    ...
    modified_coords_list = [(x, y, id_2) for (x, y, id_2) in coords_list]

    draw_colored_circles_on_image(image_path, modified_coords_list)

def frame_to_image_indexed(frame):
    image_path = "KilterBoardSetup.png"
    coords_list = frame_to_imagecoords(frame)

    # Add 2 to each id_2 value in the coords_list (as per your previous logic)
    modified_coords_list = [(x, y, id_2) for (x, y, id_2) in coords_list]
    ...
    # Draw the circles and the indices above each circle
    for idx, (x, y, z) in enumerate(modified_coords_list):
        color = hold_colors.get(z, 'black')
        draw.ellipse((x - 7, y - 7, x + 7, y + 7), fill=color, outline=color)
        # Draw the index number above the circle
        draw.text((x, y - 15), str(idx), font=font, fill="black")

    # Save the image with indexed holds
    save_path = "KilterBoardSetupIndexed.png"
    image.save(save_path)
    ...
    ax.imshow(saved_image)
    ax.axis('off')
    plt.title('Generated Climb with Hold Indices')
    plt.show()
